...
module LineFollower
  ...
  class Main
    def initialize device, proxy
      @answer = Answer.new
      @sensors = Sensors.new
      @mover = Mover.new @sensors, @answer, @commands

      @device = device
      @device.message_to_device socket.send('04INIT')
      @messages = []
      @proxy = proxy
    end
    ...
    def command cmd
      ...
      Thread.new do
        puts 'XXXXX2'
        # @device.message_to_device('04INIT')
        @device.message_to_device('18!0"0#0$0')
    ...
    def message_from_device msg
      ...
      @device.message_to_device(@answer.get)
    end
